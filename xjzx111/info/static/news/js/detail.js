            ...
            $.ajax({
                ...
                success: function (resp) {
                    if (resp.errno == "0") {
                        var comment = resp.data
                        // 拼接内容
                        // alert(resp.data.nick_name)
                        var back_html = '<div class="reply_text_con fl" >'+
                    '<div class="user_name2">'+resp.data.nick_user+'</div>'+
                    '<div class="reply_text">'+resp.data.content+ '</div>'+
                '</div>'
                        $this.parent().prevAll(".comment_time").before(back_html)
                        // 请空输入框
                        $this.prev().val('')
                        // 关闭
                        $this.parent().hide()
